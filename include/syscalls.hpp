...
_Success_(return >= 0) extern "C" DWORD GetSyscallNumber(_In_ PSYSCALL_LIST pSyscallList,
                                                         _In_ std::int32_t lFunctionHash);

PVOID GetSyscallAddress(_In_ PSYSCALL_LIST pSyscallList, _In_ std::int32_t lFunctionHash);

_Success_(return >= 0) DWORD GetSyscallHash(_In_ PSYSCALL_LIST pSyscallList, _In_ const char *szFunctionName);
